...
// ============= JERARQUÍA 3 NIVELES =============
let activeFamilia = null;
let activeLinea = null;
let activeCategoryId = null;

function toggleHierarchy() {
    const panel = document.getElementById('hierarchy-panel');
    const btn = document.getElementById('toggle-hierarchy');
    const isOpen = panel.style.display !== 'none';
    panel.style.display = isOpen ? 'none' : 'block';
    btn.classList.toggle('active', !isOpen);
}

function toggleFamilia(btn) {
    const li = btn.closest('.hie-familia');
    const subPanel = li.querySelector('.hie-lineas');
    const isOpen = subPanel && subPanel.style.display !== 'none';
    // Cerrar otros
    document.querySelectorAll('.hie-familia .hie-lineas').forEach(el => el.style.display = 'none');
    document.querySelectorAll('.hie-familia-btn').forEach(b => b.classList.remove('open'));
    if (!isOpen && subPanel) {
...
function toggleLinea(btn) {
    const group = btn.closest('.hie-linea-group');
    const subPanel = group.querySelector('.hie-cats');
    const isOpen = subPanel && subPanel.style.display !== 'none';
    document.querySelectorAll('.hie-linea-group .hie-cats').forEach(el => el.style.display = 'none');
    document.querySelectorAll('.hie-linea-btn').forEach(b => b.classList.remove('open'));
    if (!isOpen && subPanel) {
        subPanel.style.display = 'flex';
        btn.classList.add('open');
    }
...
function selectHierarchyCat(btn) {
    document.querySelectorAll('.hie-cat-btn').forEach(b => b.classList.remove('active'));
    document.querySelectorAll('.hie-familia-btn').forEach(b => b.classList.remove('active'));
    const allBtn = document.getElementById('btn-all-cats');
...
function showAllCats() {
    document.querySelectorAll('.hie-cat-btn, .hie-familia-btn').forEach(b => b.classList.remove('active'));
    const allBtn = document.getElementById('btn-all-cats');
...
// Filter by familia label click (whole familia level)
function selectFamiliaFilter(btn) {
    document.querySelectorAll('.hie-cat-btn, .hie-familia-btn').forEach(b => b.classList.remove('active'));
    const allBtn = document.getElementById('btn-all-cats');
    if (allBtn) allBtn.classList.remove('active');
    btn.classList.add('active');
...
function applyQuickFilters() {
    ...
    products.forEach(product => {
        ...
        const productFamilia = (product.getAttribute('data-familia') || '').trim();
        const productLinea = (product.getAttribute('data-linea') || '').trim();
        const productCodigo = (product.getAttribute('data-codigo') || '').toLowerCase();
        
        const searchMatch = !searchValue || productName.includes(searchValue) || productCodigo.includes(searchValue);
        let hierarchyMatch = true;
        if (activeCategoryId) {
            hierarchyMatch = productCategory === activeCategoryId;
